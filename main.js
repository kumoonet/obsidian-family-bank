...
const CHANGELOG = [
  { version: '1.1.1', date: '2026-08-07', changes: [
    '修复：资产走势图历史月份丢失（导入/恢复后只剩最近一个月），走势图现在始终从定期存款记录重建完整历史',
    '新增：结息支持两种结算方式——💵 现金结算 / 🏦 计入存款（自动转存为新定期，可选月/年定存）',
    '新增：界面语言切换（设置页可选 中文 / English），用户录入数据保持原文',
    '新增：点击版本号查看更新日志（顶部附作者署名 KuMoo · 公众号：弱者思维体系）',
    '优化：description 支持中文搜索（搜「家庭银行」可找到）'
  ] },
  { version: '1.1.0', date: '2026-08-01', changes: ['结息支持两种结算方式：现金结算 / 计入存款转存'] },
